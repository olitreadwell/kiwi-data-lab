...
```ts
import { createStatsNzClient } from '@kiwilab/stats-nz';
...
```sh
npm run test -w @kiwilab/stats-nz        # unit + integration (local stub server) + perf + security
RUN_SMOKE=1 npm run test:smoke -w @kiwilab/stats-nz   # live smoke tests against api.data.stats.govt.nz
```
